...
export async function POST(request: Request) {
  ...
  const faceDescriptor = payload.faceDescriptor ? payload.faceDescriptor : null;
  const displayNameInput = payload.displayName
    ? String(payload.displayName).trim().slice(0, 80)
    : null;
  ...
  return NextResponse.json({ voterId: voter.id });
}
